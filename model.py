...
class RCF(nn.Module):
    ...
    def forward(self, x):
        ...
        ### center crop
        crop1 = crop(so1_out, img_H, img_W)
        crop2 = crop(upsample2, img_H, img_W)
        crop3 = crop(upsample3, img_H, img_W)
        crop4 = crop(upsample4, img_H, img_W)
        crop5 = crop(upsample5, img_H, img_W)
        ### crop way suggested by liu
        # crop1 = crop_caffe(0, so1_out, img_H, img_W)
        # crop2 = crop_caffe(1, upsample2, img_H, img_W)
        # crop3 = crop_caffe(2, upsample3, img_H, img_W)
        # crop4 = crop_caffe(4, upsample4, img_H, img_W)
        # crop5 = crop_caffe(8, upsample5, img_H, img_W)
        ## upsample way
        ...
        # # Aligned cropping.
        # crop1 = so1_out[:, :, self.crop1_margin:self.crop1_margin + img_H,
        #         self.crop1_margin:self.crop1_margin + img_W]
        # crop2 = upsample2[:, :, self.crop2_margin:self.crop2_margin + img_H,
        #         self.crop2_margin:self.crop2_margin + img_W]
        # crop3 = upsample3[:, :, self.crop3_margin:self.crop3_margin + img_H,
        #         self.crop3_margin:self.crop3_margin + img_W]
        # crop4 = upsample4[:, :, self.crop4_margin:self.crop4_margin + img_H,
        #         self.crop4_margin:self.crop4_margin + img_W]
        # crop5 = upsample5[:, :, self.crop5_margin:self.crop5_margin + img_H,
        #         self.crop5_margin:self.crop5_margin + img_W]

        #print(crop1.shape)
        #print(crop2.shape)
        #fusecat = torch.cat((so1, so2, so3, so4, so5), dim=1)
        fusecat = torch.cat((crop1, crop2, crop3, crop4, crop5), dim=1)
